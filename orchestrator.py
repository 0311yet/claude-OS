...
class Orchestrator:
    ...
    def _build_prompt(self):
        ...
        state_instr = (
            ...
            "你必须在以下时机通过 Bash 工具更新此文件：\n\n"
            "1. 开始处理任务时：将 status 设为 'running'\n"
            "2. 完成一个重要步骤后：将 turn 递增 +1\n"
            "3. 全部工作完成时：写入 recovery_context（进度摘要+下一步），然后退出\n"
            "4. 遇到无法解决的阻塞时：将 status 设为 'restarting'\n\n"
            "更新方法：\n"
            "用 Bash 执行 python 读写 JSON，或用 Bash 写入完整 JSON 文件。\n\n"
            "不要将 status 设为 'idle'，除非你在等待用户手动提供信息才能继续。\n"
